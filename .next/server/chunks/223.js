...
function loadings() {
    ...
    (0,react__WEBPACK_IMPORTED_MODULE_1__.useEffect)(()=>{
        const FetchData = async ()=>{
            const { data , error  } = await _compunentConfig_supabase__WEBPACK_IMPORTED_MODULE_5__/* ["default"].from */ .Z.from("Production_history").select("Open_qty, OK_qty").filter("PD_key", "in", "(" + localStorage.getItem("PD_key") + ")").single();
            if (data) {
    ...
    //subscribe -------------------------------------------------
    (0,react__WEBPACK_IMPORTED_MODULE_1__.useEffect)(()=>{
        const ProductionHistory = _compunentConfig_supabase__WEBPACK_IMPORTED_MODULE_5__/* ["default"].channel */ .Z.channel("custom-update-Loading1").on("postgres_changes", {
            event: "UPDATE",
            schema: "public",
